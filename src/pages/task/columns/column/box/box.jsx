import {
    BoxWrap,
    Container, CreatePopup,
    Deadline, DeadlineWrap,
    DeleteIcon,
    Description,
    IconsWrap,
    Priority, PriorityWrap,
    ReplaceIcon, ReplText,
    Span,
    Title
} from "./styled";
    ...
    return (
        <Container priority={item.priority}>
            ...
            <BoxWrap>
                ...
                <DeadlineWrap>
                    <Span>Deadline</Span>
                    <Deadline>{item.deadline.toString()}</Deadline>
                </DeadlineWrap>
            </BoxWrap>
            <IconsWrap>
                        ...
                        <Back onClick={close}>
                            <CreatePopup onClick={(e) => e.stopPropagation()}>
                                {Object.values(columns)
                                    ...
                                    .map(col => (
                                        <ReplText onClick={()=>handleClick(col.id)} key={col.id}>{col.name} <ReplaceIcon/> </ReplText> // Виводимо назви інших колонок
                                    ))
                                }
                            </CreatePopup>
                        </Back>
